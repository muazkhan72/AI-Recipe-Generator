...
export const generatePantrySuggestions = async (pantryItems, expiringItems = []) => {
    ...
    const ingredient = pantryItems.map(item => item.name).join(', ');
    const expiringText = expiringItems.length > 0
         ? `\nPriority ingredients (expiring soon): ${expiringItems.join(', ')}`
         : '';

    const prompt = `Based on these available ingredients: ${ingredient}${expiringText} 
    ...
    try {
        ...
        console.error('Gemini API Error:', error);
        throw new Error('Failed to generate pantry suggestions. Please try again later.');
    }
};
...
export const generateCookingTips = async (recipe) => {
    ...
    try {
        ...
        console.error('Gemini API Error:', error);
        return ['Cook with love and patience!']; // Fallback tip
    }
};
